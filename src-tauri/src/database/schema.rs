...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute(
            ...
            enabled_hermes BOOLEAN NOT NULL DEFAULT 0,
            enabled_pi_agent BOOLEAN NOT NULL DEFAULT 0,
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    11 => {
                        log::info!("迁移数据库从 v11 到 v12（添加 Pi Coding Agent Skill 支持）");
    ...
    /// v11 -> v12：添加 Pi Coding Agent Skill 同步列。
    fn migrate_v11_to_v12(conn: &Connection) -> Result<(), AppError> {
        ...
        log::info!("v11 -> v12 迁移完成：已添加 Pi Coding Agent Skill 支持");
        Ok(())
    }
