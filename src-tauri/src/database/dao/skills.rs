...
impl Database {
        ...
        let mut stmt = conn
            .prepare(
                "SELECT id, name, description, directory, repo_owner, repo_name, repo_branch,
                        ...
                        enabled_hermes, enabled_pi_agent, installed_at, content_hash, updated_at
            ...
            .query_map([], |row| {
                Ok(InstalledSkill {
                    ...
                    apps: SkillApps {
                        ...
                        opencode: row.get(11)?,
                        hermes: row.get(12)?,
                        pi_agent: row.get(13).unwrap_or(false),
                    },
        ...
        conn.execute(
            "INSERT OR REPLACE INTO skills
             (id, name, description, directory, repo_owner, repo_name, repo_branch,
              ...
              enabled_pi_agent, installed_at, content_hash, updated_at)
    ...
    pub fn update_skill_apps(&self, id: &str, apps: &SkillApps) -> Result<bool, AppError> {
        ...
        let affected = conn
            .execute(
                "UPDATE skills SET enabled_claude = ?1, enabled_codex = ?2, enabled_gemini = ?3, enabled_opencode = ?4, enabled_hermes = ?5, enabled_pi_agent = ?6 WHERE id = ?7",
                params![
                    apps.claude,
                    apps.codex,
                    apps.gemini,
