...
export function useSettings(): UseSettingsResult {
  ...
  const resetSettings = useCallback(() => {
    ...
    resetAllDirectories({
      ...
      piAgent: sanitizeDir(data?.piAgentConfigDir),
    });
  ...
  const autoSaveSettings = useCallback(
    async (updates: Partial<SettingsFormState>): Promise<SaveResult | null> => {
      ...
      try {
        ...
        const sanitizedPiAgentDir = sanitizeDir(
          mergedSettings.piAgentConfigDir,
        );
        const {
        ...
        const payload: Settings = {
          ...
          piAgentConfigDir: sanitizedPiAgentDir,
  ...
  const saveSettings = useCallback(
    async (
      ...
      try {
        ...
        const previousPiAgentDir = sanitizeDir(data?.piAgentConfigDir);
        const {
          webdavSync: _ignoredWebdavSync,
          s3Sync: _ignoredS3Sync,
          ...restSettings
        ...
        // 如果 Claude/Codex/Gemini/OpenCode/OpenClaw/Pi Agent 的目录覆盖发生变化，则立即将"当前使用的供应商"写回对应应用的 live 配置
        ...
        const piAgentDirChanged = sanitizedPiAgentDir !== previousPiAgentDir;
        if (
          !pluginSynced &&
          (claudeDirChanged ||
            codexDirChanged ||
