...
> = {
  ...
  "pi-agent": { key: "piAgent", defaultFolder: ".pi/agent" },
};
    ...
    const load = async () => {
      try {
        ...
        setResolvedDirs({
          ...
          piAgent: piAgentDir || defaultsRef.current.piAgent,
        });
      } catch (error) {
        console.error(
  ...
  const resetAllDirectories = useCallback(
    (overrides?: ResolvedAppDirectoryOverrides) => {
      ...
      setResolvedDirs({
        ...
        piAgent: overrides?.piAgent ?? defaultsRef.current.piAgent,
      });
    },
    [],
  );
